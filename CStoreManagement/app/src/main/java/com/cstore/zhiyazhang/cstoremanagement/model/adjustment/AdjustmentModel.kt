...
class AdjustmentModel : AdjustmentInterface {
    ...
    fun createAdjustment(data: AdjustmentBean, msg: Message, ip: String, handler: MyHandler): Boolean {
        val date = CStoreCalendar.getCurrentDate(0)
        val recodeNumber = getMaxRecodeNumber(date, ip, msg, handler)
        if (recodeNumber == -1) return false
        val sql = getCreateSql(date, recodeNumber, data)
        val result = SocketUtil.initSocket(ip, sql).inquire()
        if (result != "0") {
            msg.obj = result
     ...
     * 得到最大的id用于创建
     */
    private fun getMaxRecodeNumber(date: String, ip: String, msg: Message, handler: MyHandler): Int {
     ...
     * 得到sql语句
     */
    private fun getCreateSql(date: String, recodeNumber: Int, data: AdjustmentBean): String {
        val result = StringBuilder()
        result.append(MySql.affairHeader)
        result.append(MySql.createAdjustment(data, date, recodeNumber + 1))
        result.append(MySql.affairFoot)
        return result.toString()
    }
