...
class ScrapPresenter(private val gView:GenericView,private val sView:ScrapView, private val activity:MyActivity){
    private val anInterface=ScrapModel(gView, sView)
    ...
    fun getAllScrap(){
        ...
        anInterface.getAllScrap(MyHandler.writeActivity(activity).writeListener(object : MyListener {
    ...
    fun searchScrap(message:String){
        if (!judgmentInternet(gView))return
        anInterface.searchScrap(message,MyHandler.writeActivity(activity).writeListener(object : MyListener {
            override fun listenerSuccess(data: Any) {
                gView.requestSuccess(data as ArrayList<ScrapContractBean>)
            }
    ...
    fun submitScraps(data:ArrayList<ScrapContractBean>, reCode:Int){
        if (!judgmentInternet(gView))return
        anInterface.submitScraps(data,reCode, MyHandler.writeActivity(activity).writeListener(object : MyListener {
            override fun listenerSuccess(data: Any) {
                sView.updateDone()
