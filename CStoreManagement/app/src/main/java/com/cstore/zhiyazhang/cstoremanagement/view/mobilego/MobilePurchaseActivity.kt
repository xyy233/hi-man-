 ...
 * on 2018/6/21 10:21.
 */
class MobilePurchaseActivity(override val layoutId: Int = R.layout.activity_contract) : MyActivity(), EasyPermissions.PermissionCallbacks {
    ...
    override fun initView() {
        ...
        toolbar.title = getString(R.string.purchase)
        setSupportActionBar(toolbar)
        initGun()

        swipe_recycler.layoutManager = MyLinearlayoutManager(this@MobilePurchaseActivity, LinearLayoutManager.VERTICAL, false)
    ...
    override fun initClick() {
        ...
        qrcode.setOnClickListener {
            if (android.os.Build.VERSION.SDK_INT >= 23) {
                if (judgmentCarmer()) {
                    val i = Intent(this@MobilePurchaseActivity, ContractSearchActivity::class.java)
                ...
                if (cameraIsCanUse()) {
                    ...
                    intent.data = Uri.fromParts("package", this@MobilePurchaseActivity.packageName, null)
                    this@MobilePurchaseActivity.startActivity(intent)
                }
            }
        }
            ...
            if (adapter.data.size < 1) {
                ...
                val d = MobileDetailBean(null, driverCode, null, 1.0, "0", ArrayList())
                adapter.data.forEach {
                    d.detail.add(MobileDetailItemBean(it.pluId, it.goodsNum!!, 0))
                }
    ...
    override fun <T> errorDealWith(eData: T) {
        AlertDialog.Builder(ContextThemeWrapper(this, R.style.AlertDialogCustom))
                .setTitle("提示")
                .setMessage("补货异常！${eData.toString()}")
