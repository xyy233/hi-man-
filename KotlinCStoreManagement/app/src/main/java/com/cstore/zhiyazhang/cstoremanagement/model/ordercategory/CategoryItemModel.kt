...
class CategoryItemModel : CategoryInterface {
     ...
     * 根据新品档期获得商品
     */
    override fun getNewItemById(nop: NOPBean, orderBy: String, myListener: MyListener) {
        val ip = MyApplication.getIP()
        if (ip == MyApplication.instance().getString(R.string.notFindIP)) {
            myListener.listenerFailed(ip)
            return
        ...
        var sql: String? = null
        if (nop.nopId == "0") sql = MySql.getPromotion(orderBy) else sql = MySql.getNewItemById(nop.nopId, orderBy)
...
interface CategoryInterface {
     ...
     * 根据新品档期获得商品或直接获得促销品
     */
    fun getNewItemById(nop: NOPBean, orderBy: String, myListener: MyListener)
